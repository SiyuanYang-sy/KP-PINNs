...
# one-dimensional  X
# A, Phi = KP_compute_APhi(X, 1, 1/2) 
# A, Phi = KP_compute_APhi(X, 1, 3/2)
# A, Phi = KP_compute_APhi(X, 1, 5/2)

# b = [Phi(X)]^(-1) Y, Phi(X) b = Y
# b = spsolve(Phi, Y)    
# [Y^T][K^(-1)]Y = Y.T @ (A @ b)


def KP_compute_APhi(X, theta, nu):  # theta = 1, nu = 1/2, 3/2, 5/2
    if nu == 1/2:   # k = 2*nu + 2 = 3
        n = X.shape[0]
        X = X * theta
        vec1 = X[1:] - X[:-1]
    ...
    # return A_csr, Phi_csr + 0.1 * A_csr
